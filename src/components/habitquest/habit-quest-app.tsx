...
export function HabitQuestApp() {
  ...
  const todayReward = useMemo(
    () =>
      hydrated
        ...
        : null,
    [achievements, challenges, completions, dailyRewards, equippedItems, habits, hydrated, levelUnlocks, shopItems, userProgress, wallet],
  );
  ...
  const completedHabitIds = useMemo(
    () =>
      new Set(
        todaysHabits
          .filter((habit) => hasCompletionForDate(completions, habit.id, todayReward?.dateKey ?? ""))
          .map((habit) => habit.id),
      ),
  ...
  const weeklyActivity = useMemo(
    ...
    [achievements, challenges, completions, dailyRewards, equippedItems, habits, levelUnlocks, shopItems, userProgress, wallet],
  );
      ...
      <main className="grid gap-6 pt-6">
        ...
        <div className="grid gap-6 lg:grid-cols-4">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="glass-panel h-36 animate-pulse rounded-[2rem] bg-white/5" />
          ))}
        </div>
      </main>
    );
  ...
  return (
    <main className="grid gap-6 pt-6">
      ...
      >
        <GlassCard className="panel-highlight overflow-hidden rounded-[2rem] p-6 md:p-8">
          <div className="grid gap-8 xl:grid-cols-[1.4fr_0.9fr]">
            <div className="space-y-4">
              <div className="flex flex-wrap items-center gap-3">
                <span className="rounded-full border border-cyan-300/30 bg-cyan-300/10 px-4 py-1 text-xs uppercase tracking-[0.28em] text-cyan-200">
                  HabitQuest
                </span>
                <span className="rounded-full border border-white/10 bg-white/5 px-4 py-1 text-xs uppercase tracking-[0.28em] text-[var(--color-text-muted)]">
                  Advanced Gamification
                </span>
              </div>
              <div className="space-y-3">
                <h1 className="section-title max-w-3xl text-4xl leading-tight text-white md:text-6xl">
                  Progression that pays you back for consistency.
                </h1>
                <p className="max-w-2xl text-base leading-7 text-[var(--color-text-muted)] md:text-lg">
            ...
            <div className="glass-panel rounded-[1.75rem] border border-white/10 p-5">
              <div className="mb-4 flex items-center justify-between gap-3">
                ...
                <div className="rounded-3xl border border-amber-300/20 bg-amber-300/10 px-4 py-3 text-center">
                  <p className="text-xs uppercase tracking-[0.24em] text-amber-200/80">Coins</p>
