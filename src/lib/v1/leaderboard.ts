...
export async function getLevelLeaderboardAction() {
  ...
  const rows = await database
    ...
    .orderBy(
      desc(userProgress.currentStreak),
      desc(userProgress.bestStreak),
      desc(userProgress.totalExp),
    )
  ...
  if (!you) {
    ...
    if (mine) {
      const [ahead] = await database
        ...
        .where(
          or(
            gt(userProgress.currentStreak, mine.currentStreak),
            and(
              eq(userProgress.currentStreak, mine.currentStreak),
              gt(userProgress.bestStreak, mine.bestStreak),
            ),
            and(
              eq(userProgress.currentStreak, mine.currentStreak),
              eq(userProgress.bestStreak, mine.bestStreak),
