...
/**
 * L2–L30 track totaling 71 coins — small steps with milestone bumps and an L30 finale.
 */
export function buildSeasonPassRewards(): SeasonPassReward[] {
  const rewards: SeasonPassReward[] = [];
  for (let level = 2; level <= SEASON_PASS_MAX_LEVEL; level += 1) {
    const isFinale = level === SEASON_PASS_MAX_LEVEL;
    const isMilestone = !isFinale && level % 5 === 0;
    const coins = isFinale ? 10 : isMilestone ? 3 : 2;
    const exp = isFinale ? 80 : isMilestone ? 35 : 18;
    const label = isFinale
      ? "Season finale"
      : isMilestone
        ? `Season crest ${level}`
        : `Season tier ${level}`;
    rewards.push({ level, coins, exp, label });
  }
  return rewards;
}
  ...
  return [
    {
      ...
      completed: false,
      claimed: false,
      reward: { coins: 10, exp: 80, unlockThemeId: null },
    },
    {
      id: createId("quest"),
      key: "arc-tempering",
      ...
      completed: false,
      claimed: false,
      reward: { coins: 18, exp: 120, unlockThemeId: "theme_ember" },
    },
    {
      id: createId("quest"),
      key: "arc-ascension",
      ...
      completed: false,
      claimed: false,
      reward: { coins: 28, exp: 200, unlockThemeId: "theme_aurora" },
    },
  ];
}
...
export function createSeasonPass(monthKey = getStartOfCurrentMonthKey()): SeasonPassState {
  return {
    ...
    level: 1,
    claimedLevels: [],
    rewards: buildSeasonPassRewards(),
  };
}
...
export function addSeasonPassXp(pass: SeasonPassState, amount: number) {
  const nextXp = Math.max(0, pass.xp + amount);
  const level = Math.min(
    SEASON_PASS_MAX_LEVEL,
    Math.max(1, Math.floor(nextXp / SEASON_PASS_XP_PER_LEVEL) + 1),
  );
  return {
    ...pass,
    xp: nextXp,
